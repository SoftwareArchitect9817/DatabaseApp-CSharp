...
namespace Senaka
{
    public partial class ColourShipping : Form
    {
        ...
        private void scanInput(string data)
        {
            ...
            if (exist_prefix)
            {
                ...
                if (frame_cutting_data != null)
                {
                    ...
                    List<string[]> colour_shipping = DB.fetchRows("ColourShipping", "id", id);
                    int scanned_qty = colour_shipping.Count;
                    if (scanned_qty < 1)
                    {
                        //   string rack_id = frame_cutting_data[(int)GLASS.RACK_ID];
                        if (DB.saveColourShippingData(id, date, time, name, batch_number) == 0)
                        {
                            bool exists = false;
