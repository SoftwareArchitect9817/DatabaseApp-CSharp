...
namespace Senaka
{
    public partial class ColourReceiving : Form
    {
        ...
        {
            public Data_order( string order_number, int cs_f, int cs_s, int s_f, int l_f, int sl_f, int sl_s, int bmd, int bmd_done, int cs_f_done, int cs_s_done, int s_f_done, int l_f_done, int sl_f_done, int sl_s_done,string bmd_info, string cs_f_info, string cs_s_info, string s_f_info, string l_f_info, string sl_f_info, string sl_s_info,string delivered_info)
            {
          ...
          public string Delivered_info { get; set; }

        }

        List<Data_order> list = new List<Data_order>();
        
        int cs_f_total = 0, cs_s_total = 0, l_f_total = 0, s_f_total = 0, sl_f_total = 0, sl_s_total = 0, bmd_total = 0, cs_f_delivered = 0, cs_s_delivered = 0, s_f_delivered = 0, l_f_delivered = 0, sl_f_delivered = 0, sl_s_delivered = 0, bmd_delivered = 0;
        string deliver = "";
        public ColourReceiving()
        {
            ...
            error_timer.Interval = Settings.Colour_Shipping_Error_Time * 1000;
        ...
        private void ColourReceiving_FormClosing(object sender, FormClosingEventArgs e)
        {
            timer.Stop();
            error_timer.Stop();
        ...
        private void scanInput(string data)
        {
            ...
            foreach (string[] r in Settings.Colour_Shipping_Prefix_Table)
            {
            ...
            if (exist_prefix)
            {
                string name = prefix_data[4], batch_number = textBoxBatchNumber.Text, date = now.ToString("yyyy-MM-dd"), time = now.ToString("HH:mm:ss");
                frame_cutting_data = DB.fetchRow("framescutting", "F", id, false);
                if (frame_cutting_data != null)
                {

                    List<string[]> colour_receiving = DB.fetchRows("ColourReceiving", "id", id);
                    int scanned_qty = colour_receiving.Count;
                    if (scanned_qty < 1)
                    {
                        //   string rack_id = frame_cutting_data[(int)GLASS.RACK_ID];
                        if (DB.saveColourReceivingData(id, date, time, name, batch_number) == 0)
                        {
                            bool exists = false, sash_insert = false;
                            List<string[]> frame_cutting_data_sash;
                            string sash_id="";
                            if (Settings.CountSashFrame_boolean == "True")
                            {
                                if (frame_cutting_data[7] == "Case Frame")
                                {
                                    frame_cutting_data_sash = DB.getFrameCuttingByIdAndType(frame_cutting_data[9]);
                                    if (frame_cutting_data_sash != null)
                                    {
                                        List<string> ids = new List<string>();
                                        foreach (var element in frame_cutting_data_sash)
                                            ids.Add(element[5]);
                                            List<string[]> colour_receiving_sash = DB.importColourReceivingByIDs(ids);
                                        List<string> ids_done = new List<string>();
                                        foreach (var element in colour_receiving_sash)
                                            ids_done.Add(element[0]);
                                        foreach (var element in frame_cutting_data_sash)
                                            if (!ids_done.Contains(element[5]))
                                            {
                                                sash_id = element[5];
                                               
                                                    if (DB.saveColourReceivingData(sash_id, date, time, name,batch_number) == 0) sash_insert = true;
                                                break;
                                            }
                                    }
                                }
                            ...
                            for (int j = 0; j < list.Count; j++)
                            {
                                if (list[j].Order_numb == frame_cutting_data[9])
                                {
                                    ...
                                    else if (Settings.Large_Fix.Any(type => type[2].Equals(frame_cutting_data[7], StringComparison.InvariantCultureIgnoreCase)))
                                    {
                                     ...
                                     if (Settings.Casement_Sash.Any(type => type[2].Equals(frame_cutting_data[7], StringComparison.InvariantCultureIgnoreCase)) || sash_insert)
                                    {
                            ...
                            if (!exists)
                            {
                                var line_numbs = new List<string>();
                                ...
                                for (int j = 0; j < frame_cutting_order_data.Count; j++)
                                {
                                    ...
                                    if (Settings.Casement_Sash.Any(type => type[2].Equals(frame_cutting_order_data[j][7], StringComparison.InvariantCultureIgnoreCase)))
                                    {

                                        cs_s += 1;

                                    }
                                    line_numbs.Add(frame_cutting_order_data[j][5]);
                                }

                                List<string[]> colour_shipping_delivered = DB.getColourShippingDelivered(line_numbs);

                                List<string[]> colour_receiving_done = DB.importColourReceivingByIDs(line_numbs);
                                List<string> ids_total_done = new List<string>();
                                foreach (var element in colour_receiving_done)
                                    ids_total_done.Add(element[0]);
                                //done
                                for (int j = 0; j < frame_cutting_order_data.Count; j++)
                                {
                                  if(ids_total_done.Contains(frame_cutting_order_data[j][5]))  { 
                                    if (Settings.Brickmould.Any(type => type[2].Equals(frame_cutting_order_data[j][7].ToString(), StringComparison.InvariantCultureIgnoreCase)))
                                    {
                                        bmd_done += 1;
                                    }
                                ...
                                for (int j = 0; j < frame_cutting_order_data.Count; j++)
                                {
                                    if (Settings.Brickmould.Any(type => type[2].Equals(frame_cutting_order_data[j][7].ToString(), StringComparison.InvariantCultureIgnoreCase)) && colour_shipping_delivered.Any(order => order[0].Equals(frame_cutting_order_data[j][5].ToString(), StringComparison.InvariantCultureIgnoreCase)))
                                    {
                                        bmd_delivered += 1;
                                    }
                                    else if (Settings.Casement_Frame.Any(type => type[2].Equals(frame_cutting_order_data[j][7], StringComparison.InvariantCultureIgnoreCase)) && colour_shipping_delivered.Any(order => order[0].Equals(frame_cutting_order_data[j][5].ToString(), StringComparison.InvariantCultureIgnoreCase)))
                                    {
                                    ...
                                    else if (Settings.Slider_Frame.Any(type => type[2].Equals(frame_cutting_order_data[j][7], StringComparison.InvariantCultureIgnoreCase)) && colour_shipping_delivered.Any(order => order[0].Equals(frame_cutting_order_data[j][5].ToString(), StringComparison.InvariantCultureIgnoreCase)))
                                    {
                                        sl_f_delivered += 1;

                                    }
                                    else if (Settings.Slider_sash.Any(type => type[2].Equals(frame_cutting_order_data[j][7], StringComparison.InvariantCultureIgnoreCase)) && colour_shipping_delivered.Any(order => order[0].Equals(frame_cutting_order_data[j][5].ToString(), StringComparison.InvariantCultureIgnoreCase)))
                                    {

                                        sl_s_delivered += 1;

                                    }
                                    else if (Settings.Large_Fix.Any(type => type[2].Equals(frame_cutting_order_data[j][7], StringComparison.InvariantCultureIgnoreCase)) && colour_shipping_delivered.Any(order => order[0].Equals(frame_cutting_order_data[j][5].ToString(), StringComparison.InvariantCultureIgnoreCase)))
                                    {
                                        l_f_delivered += 1;

                                    }
                                    else if (Settings.Small_Fix.Any(type => type[2].Equals(frame_cutting_order_data[j][7], StringComparison.InvariantCultureIgnoreCase)) && colour_shipping_delivered.Any(order => order[0].Equals(frame_cutting_order_data[j][5].ToString(), StringComparison.InvariantCultureIgnoreCase)))
                                    {
                                        s_f_delivered += 1;

                                    }
                                     if (Settings.Casement_Sash.Any(type => type[2].Equals(frame_cutting_order_data[j][7], StringComparison.InvariantCultureIgnoreCase)) && colour_shipping_delivered.Any(order => order[0].Equals(frame_cutting_order_data[j][5].ToString(), StringComparison.InvariantCultureIgnoreCase)) || sash_insert)
                                    {
                                ...
                                deliver = cs_f_delivered + "/" + cs_s_delivered + "/" + s_f_delivered + "/" + l_f_delivered + "/" + sl_f_delivered + "/" + sl_s_delivered + "/" + bmd_delivered;
                               
                               
                                    list.Add(new Data_order(frame_cutting_data[9], cs_f, cs_s, s_f,l_f, sl_f, sl_s, bmd, bmd_done, cs_f_done, cs_s_done, s_f_done, l_f_done, sl_f_done, sl_s_done, bmd_done+"/"+bmd, cs_f_done + "/" + cs_f, cs_s_done + "/" + cs_s, s_f_done + "/" + s_f, l_f_done + "/" + l_f, sl_f_done + "/" + sl_f, sl_s_done + "/" + sl_s, deliver));
                            ...
                            if(sash_insert) ScanedData.Rows.Insert(0, sash_id, date, time, name, batch_number);
        ...
        private void OrderScanedData_SelectionChanged_2(object sender, EventArgs e)
        {
