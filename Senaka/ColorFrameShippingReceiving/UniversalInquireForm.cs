...
namespace Senaka
{
    public partial class UniversalInquireForm : Form
    {
        string _Type;
        DataGridViewColumnCollection _Columns;
        public UniversalInquireForm(List<string[]> data,string type,bool close=true)
        {
            InitializeComponent();
            if(close)
        ...
        public void Set(List<string[]> data, string type, bool close = true)
        {
            if (data != null)
            {
                ...
                List<string[]> data_done_shipping = new List<string[]>(), data_done_receiving = new List<string[]>(), Prefix_date = new List<string[]>();

                switch (type)
                {
                    case "DVCoatexColor":
                        this.Text = "DVCoatex Color";
                    if (frame_ids.Count != 0)
                    {
                        data_done_shipping = DB.importDVCoatexColorShippingByIds(frame_ids);
                        data_done_receiving = DB.importDVCoatexColorReceivingByIds(frame_ids);
                    }
                    ...
                    if (names.Count != 0)
                       Prefix_date = DB.importDVCoatexColorShippingPrefixByNames(names);
                    
                        break;
                    case "ExpressCoatingColor":
                        this.Text = "Express Coating Color";
                        if (frame_ids.Count != 0)
                        {
                            data_done_shipping = DB.importExpressCoatingColorShippingByIds(frame_ids);
                            data_done_receiving = DB.importExpressCoatingColorReceivingByIds(frame_ids);
                        }
                        foreach (var element in data_done_shipping)
                        {
                            names.Add(element[3]);
                        ...
                        if (names.Count != 0)
                            Prefix_date = DB.importExpressCoatingColourShippingPrefixByNames(names);

                        break;
                    case "VinylproFrame":
                        this.Text = "Vinylpro Frame";
                        if (frame_ids.Count != 0)
                        {
                        ...
                        if (names.Count != 0)
                            Prefix_date = DB.importVinylProFrameShippingPrefixByNames(names);
                ...
                foreach (var element in data)
                {
                    ...
                    {
                        var match_shipping_prefix = Prefix_date.FirstOrDefault(stringToCheck => stringToCheck[4].Contains(match_shipping[4]));
                        if (match_shipping_prefix != null)

                            company = match_shipping_prefix[3];
                        shipping_name = match_shipping[4];
                        shipping_date = match_shipping[2].Substring(0, 10);
                    ...
                    if (match_receiving != null)
                    {
                        ...
                        receiving_date = match_receiving[2].Substring(0, 10);
