...
namespace Senaka
{
    public partial class DVCoatexColorShipping : Form
    {
        ...
        public DVCoatexColorShipping()
        {
            ...
            error_timer.Interval = Settings.DVCoatex_Color_Shipping_Error_Time * 1000;
        ...
        private void scanInput(string data)
        {
            ...
            foreach (string[] r in Settings.DVCoatex_Color_Shipping_Prefix_Table)
            {
            ...
            if (exist_prefix)
            {
                ...
                if (frame_cutting_data != null)
                {
                    ...
                    List<string[]> dvcoatex_shipping = DB.fetchRows("DVCoatexColorShipping", "line", id);
                    ...
                    if (scanned_qty < 1)
                    {
                        if (frame_cutting_data[16] != "WHT" || frame_cutting_data[17] != "WHT")
                        {
                            ...
                            if (DB.saveDVCoatexColorShippingData(id, date, time, name, batch_number) == 0)
                            {
                                bool exists = false;
