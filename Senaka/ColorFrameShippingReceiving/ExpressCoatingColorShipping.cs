...
namespace Senaka
{
    public partial class ExpressCoatingColorShipping : Form
    {
        ...
        public ExpressCoatingColorShipping()
        {
            InitializeComponent();
            ...
            error_timer.Interval = Settings.ExpressCoating_Color_Shipping_Error_Time * 1000;
        ...
        private void scanInput(string data)
        {
            ...
            foreach (string[] r in Settings.ExpressCoating_Color_Shipping_Prefix_Table)
            {
            ...
            if (exist_prefix)
            {
                ...
                if (frame_cutting_data != null)
                {
                    ...
                    List<string[]> expressCoating_shipping = DB.fetchRows("ExpressCoatingColourShipping", "line", id);
                    int scanned_qty = expressCoating_shipping.Count;
                    if (scanned_qty < 1)
                    {
                        if (frame_cutting_data[16] != "WHT" || frame_cutting_data[17] != "WHT")
                        {
                            //   string rack_id = frame_cutting_data[(int)GLASS.RACK_ID];
                            if (DB.saveExpressCoatingColorShippingData(id, date, time, name, batch_number) == 0)
                            {
                                bool exists = false;
