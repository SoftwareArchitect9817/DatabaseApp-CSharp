...
namespace Senaka
{
    public partial class DVCoatexColorReceiving : Form
    {
        ...
        public DVCoatexColorReceiving()
        {
            ...
            error_timer.Interval = Settings.DVCoatex_Color_Receiving_Error_Time * 1000;
        ...
        private void scanInput(string data)
        {
            ...
            foreach (string[] r in Settings.DVCoatex_Color_Receiving_Prefix_Table)
            {
            ...
            if (exist_prefix)
            {
                ...
                if (frame_cutting_data != null)
                {
                    ...
                    List<string[]> dvcoatex_receiving = DB.fetchRows("DVCoatexColorReceiving", "line", id);
                    int scanned_qty = dvcoatex_receiving.Count;
                    if (scanned_qty < 1)
                    {
                        if (frame_cutting_data[16] != "WHT" || frame_cutting_data[17] != "WHT")
                        {
                            //   string rack_id = frame_cutting_data[(int)GLASS.RACK_ID];
                            if (DB.saveDVCoatexColorReceivingData(id, date, time, name, batch_number) == 0)
                            {
                                bool exists = false;
