...
namespace Senaka
{
    public partial class PatioDoorsReceiving : Form
    {
        ...
        private void scanInput(string data)
        {
            ...
            foreach (string[] r in Settings.PatioDoors_Receiving_Prefix_Table)
            {
            ...
            if (exist_prefix)
            {
                string gate = prefix_data[2];
                List<string[]> Vista_Total = new List<string[]>(), Oceanview_Total = new List<string[]>();
                string[] vista = DB.fetchRow("VistaPatioDoors", "Door number", door_number);
                string[] oceanView = DB.fetchRow("OceanviewPatioDoors", "Door number", door_number);
                if (vista != null || oceanView != null)
                {
                    string[] PatioDoorsReceiving = DB.fetchRow("PatioDoorsReceiving", "Door_number", door_number);
                    if (PatioDoorsReceiving == null)
                    {
                        if (DB.savePatioDoorsReceivingData(door_number, date, time, gate, name) == 0)
                        {
                            string CompanyName = "";
                            ...
                            CompanyName = vista != null ? vista[30] : oceanView != null ? oceanView[21] : null;
                            ...
                            if (!exist)
                            {
                                ...
                                int scanned_doors = DB.importPatioDoorsReceivingByIds(ids).Count;
                            ...
                            iShippingScanedData.Rows.Add(door_number, date, time, name, gate);
