...
namespace Senaka
{
    public partial class FrameShippingReportForm : Form
    {
        private int m_currentPageIndex;
        private IList<Stream> m_streams;
        Timer blink_timer;
        ReportParameterCollection reportParameters = new ReportParameterCollection();
        List<string[]> ColorShippingDB = new List<string[]>();
        List<string[]> VinylProShippingDB = new List<string[]>();
        ...
        List<dataOrder> ShippingFrames = new List<dataOrder>();

        public FrameShippingReportForm(List<string[]> ordersummaryData, DateTime startDate, DateTime endDate)
        {
            InitializeComponent();
        ...
        private void insertData(List<string[]> FramesDB, Boolean isWhite, int idIndex, List<string[]> ordersummaryData)
        {
            for (int i = 0; i < FramesDB.Count; i++)
            {
                ...
                int index = ShippingFrames.FindIndex(x => x.Order_numb == currOrdNumber && x.isWhite == isWhite);
        ...
        private void initForm(List<string[]> ordersummaryData)
        {
            ...
            VinylProShippingDB = DB.getVinylProShipping(ordNumbers);
            insertData(VinylProShippingDB, true, 5, ordersummaryData);

            ordNumbers = ordersummaryData.FindAll(x => x[1] != "WHT" || x[2] != "WHT").Select(x => x[0]).ToList();
            ColorShippingDB = DB.getColorFrameShipping(ordNumbers);
            insertData(ColorShippingDB, false, 5, ordersummaryData);
          
            for (int i = 0; i < ShippingFrames.Count; i++)
            {
                bool cs_f = false, cs_s = false, sm_f = false, sl_f = false, sl_s = false, bmd = false, lg_f = false;
                string status;
                int currentDone = 0, currentTotal = 0;
                ShippingFrames[i].Bmd_info = ShippingFrames[i].Bmd_done + "/" + ShippingFrames[i].Bmd_total;
                ShippingFrames[i].Cs_F_info = ShippingFrames[i].Cs_F_done + "/" + ShippingFrames[i].Cs_F_total;
                ShippingFrames[i].Cs_S_info = ShippingFrames[i].Cs_S_done + "/" + ShippingFrames[i].Cs_S_total;
                ShippingFrames[i].Lg_F_info = ShippingFrames[i].Lg_F_done + "/" + ShippingFrames[i].Lg_F_total;
                ShippingFrames[i].Sl_F_info = ShippingFrames[i].Sl_F_done + "/" + ShippingFrames[i].Sl_F_total;
                ShippingFrames[i].Sl_S_info = ShippingFrames[i].Sl_S_done + "/" + ShippingFrames[i].Sl_S_total;
                ShippingFrames[i].Sm_F_info = ShippingFrames[i].Sm_F_done + "/" + ShippingFrames[i].Sm_F_total;

                bmd_TotalDone += ShippingFrames[i].Bmd_done;
                cs_f_TotalDone += ShippingFrames[i].Cs_F_done;
                cs_s_TotalDone += ShippingFrames[i].Cs_S_done;
                lg_f_TotalDone += ShippingFrames[i].Lg_F_done;
                sl_f_TotalDone += ShippingFrames[i].Sl_F_done;
                sl_s_TotalDone += ShippingFrames[i].Sl_S_done;
                sm_f_TotalDone += ShippingFrames[i].Sm_F_done;

                currentDone += ShippingFrames[i].Bmd_done
                    + ShippingFrames[i].Cs_F_done + ShippingFrames[i].Cs_S_done + ShippingFrames[i].Sl_F_done
                    + ShippingFrames[i].Lg_F_done + ShippingFrames[i].Sl_S_done + ShippingFrames[i].Sm_F_done;

                currentTotal += ShippingFrames[i].Bmd_total
                    + ShippingFrames[i].Cs_F_total + ShippingFrames[i].Cs_S_total + ShippingFrames[i].Sl_F_total
                    + ShippingFrames[i].Lg_F_total + ShippingFrames[i].Sl_S_total + ShippingFrames[i].Sm_F_total;

                totalFrames += currentTotal;
                doneFrames += currentDone;
               
                if (ShippingFrames[i].Bmd_done == ShippingFrames[i].Bmd_total) bmd = true;
                if (ShippingFrames[i].Cs_F_done == ShippingFrames[i].Cs_F_total) cs_f = true;
                if (ShippingFrames[i].Cs_S_done == ShippingFrames[i].Cs_S_total) cs_s = true;
                if (ShippingFrames[i].Lg_F_done == ShippingFrames[i].Lg_F_total) lg_f = true;
                if (ShippingFrames[i].Sl_F_done == ShippingFrames[i].Sl_F_total) sl_f = true;
                if (ShippingFrames[i].Sl_S_done == ShippingFrames[i].Sl_S_total) sl_s = true;
                if (ShippingFrames[i].Sm_F_done == ShippingFrames[i].Sm_F_total) sm_f = true;

                if (cs_f && cs_s && sm_f && sl_f && sl_s && bmd && lg_f) status = "COMPLETE";
                else if (currentDone != 0) status = "IN PROGRESS";
                else status = "NOT READY";

                ShippingFrames[i].Status = status;
                ShippingFrames[i].Info = (double)currentDone / (double)currentTotal * 100;
            }

            List<string[]> NotFoundOrders = ordersummaryData.Where(p => !ShippingFrames.Any(p2 => p2.Order_numb == p[0]))
            ...
            ProductionReportData.DataSource = ShippingFrames;
        }
        ...
        private void buttonPrint_Click(object sender, EventArgs e)
        {
            ...
            path = Path.Combine(Environment.CurrentDirectory, @"reports\FrameClearingReport.rdlc");
        ...
        private void buttonPrintNotComplete_Click(object sender, EventArgs e)
        {
            ...
            IEnumerable<string> LineNumbersVinyl = from order in VinylProShippingDB
                                                   where order[5] == null
                                                  select order[0];
            ...
            {
                string name = "", date = "", machine_id = "", time = "", status = "Vinyl pro shipping";
                data_Order.Tables[0].Rows.Add(
                    element[10],
                    element[12],
                    element[13],
            ...
            IEnumerable<string> LineNumbersColor = from order in ColorShippingDB
                                                   where order[5] == null
                                                   select order[0];
            ...
            {
                string name = "", date = "", machine_id = "", time = "", status = "Color shipping";
                data_Order.Tables[0].Rows.Add(
                    element[10],
                    element[12],
                    element[13],
        ...
        private void ProductionReportData_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == ProductionReportData.Columns["Detail"].Index && e.RowIndex >= 0)
            {
                ...
                {
                    InquireForm inquireForm = new InquireForm(data, "VinylFrameShipping", false);
                ...
                {
                    InquireForm inquireForm = new InquireForm(data, "ColourShipping", false);
                    inquireForm.FormClosed += new FormClosedEventHandler(Form1_FormClosed);
                    inquireForm.ShowDialog();
                }
            }
