...
namespace DGVPrinterHelper_report //AllocationRequest
{    
    ...
    public static class Extensions
    {
            ...
            Graphics g, int pagewidth, int pageheight, Margins margins)
        {
            foreach (T t in list)
            {
                if (t is DGVPrinter_report.ImbeddedImage)
                {
                    DGVPrinter_report.ImbeddedImage ii = (DGVPrinter_report.ImbeddedImage)Convert.ChangeType(t, typeof(DGVPrinter_report.ImbeddedImage));
                    ...
                    /*  g.DrawImage(ii.theImage, 150,180,
                    ...
                    g.DrawLine(new Pen(brush), margins.Left, 150, pagewidth+margins.Right, 150);
                }
            }
        }
    ...
    /// </summary>
    public class DGVPrinter_report
    {
        ...
        protected float imagheight;
        //private List<List<SizeF>> cellsizes;
        public float imageHeight
        {
            get { return imagheight; }
            set { imagheight = value; }
        }
        #endregion
        ...
        private float HeaderHeight 
        {
            get
            {
                ...
                // already included
                headerheight += TitleHeight+ SubTitleHeight+imageHeight;
        ...
        private Boolean? columnHorizontal;
        public Boolean? ColumnHorizontal
        {
            get { return columnHorizontal; }
        ...
        public void SendEmail(DataGridView dgv)
        {
            if (EnableLogging) Logger.LogInfoMsg("PrintNoDisplay process started");
            if (null == dgv) throw new Exception("Null Parameter passed to DGVPrinter.");
            if (!(dgv is DataGridView))
            ...
            for (i = 0; i < colstoprint.Count; i++)
            {
                ...
                if(col.DataPropertyName!="Status")
                    colwidths.Add(col.Width);     
                        ...
                        colwidths.Add(120);      // otherwise use the data width
                }
                else
                {
                    usewidth = printWidth;
               ...
               if(ColumnHorizontal== false) colheaderheight = 80;
            }
            ...
            DataGridViewCell cell, float startlocation, StringFormat cellformat, Pen lines)
        {
            ...
            if (!DrawOwnerDrawCell(g, cell.RowIndex, cell.ColumnIndex, cellprintarea, style))
            {
                ...
                {
                    if (columnHorizontal == true)
                        g.DrawString(cell.FormattedValue.ToString(), style.Font,
                         new SolidBrush(style.ForeColor), actualprint, cellformat);
                    else
                    {
                        if (cell.FormattedValue.ToString() != "Name")
                        {
                            g.TranslateTransform(actualprint.X, actualprint.Y + actualprint.Height);
                            g.RotateTransform(-90);
                            // draw the text for the cell at the row / col intersection
                            g.DrawString(cell.FormattedValue.ToString(), style.Font,
                                new SolidBrush(style.ForeColor), 0, 0);
                            g.ResetTransform();
                        }
                        else
                            g.DrawString(cell.FormattedValue.ToString(), style.Font,
